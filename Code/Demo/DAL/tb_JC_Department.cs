...
namespace Demo.DAL
{
	...
	public partial class tb_JC_Department
	{
		...
		public bool Exists(string ID)
		{
			...
			strSql.Append("select count(1) from tb_JC_Department");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(Demo.Model.tb_JC_Department model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tb_JC_Department(");
			strSql.Append("ID,DepartmentNO,DepartmentName,Status)");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Demo.Model.tb_JC_Department model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update tb_JC_Department set ");
		...
		public bool Delete(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_JC_Department ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.NVarChar,64)			};
			parameters[0].Value = ID;
		...
		/// 得到一个对象实体
		/// </summary>
		public Demo.Model.tb_JC_Department GetModel(string ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,DepartmentNO,DepartmentName,Status from tb_JC_Department ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.NVarChar,64)			};
			parameters[0].Value = ID;
		...
		/// 得到一个对象实体
		/// </summary>
		public Demo.Model.tb_JC_Department DataRowToModel(DataRow row)
		{
			Demo.Model.tb_JC_Department model=new Demo.Model.tb_JC_Department();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,DepartmentNO,DepartmentName,Status ");
			strSql.Append(" FROM tb_JC_Department ");
			if(strWhere.Trim()!="")
			{
        ...
        public DataSet GetListByPage(string sqlWhere, string order, int pageSize, int pageIndex, out long totalRecord)
		{
            ...
            DataSet ds = pageData.getDataPage("tb_JC_Department", "*", sqlWhere, order, pageSize, pageIndex, out totalRecord);
            return ds;
		}
