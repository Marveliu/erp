...
namespace Demo.DAL
{
	...
	public partial class tb_JC_BOM
	{
		...
		public bool Exists(string ID)
		{
			...
			strSql.Append("select count(1) from tb_JC_BOM");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(Demo.Model.tb_JC_BOM model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tb_JC_BOM(");
			strSql.Append("ID,MaterialNO,MaterialType,CheckNO,CheckStatus,CheckDate,Status)");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Demo.Model.tb_JC_BOM model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update tb_JC_BOM set ");
			...
			parameters[2].Value = model.CheckNO;
		...
		public bool Delete(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_JC_BOM ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.NVarChar,64)			};
			parameters[0].Value = ID;
		...
		/// 得到一个对象实体
		/// </summary>
		public Demo.Model.tb_JC_BOM GetModel(string ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,MaterialNO,MaterialType,CheckNO,CheckStatus,CheckDate,Status from tb_JC_BOM ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.NVarChar,64)			};
			parameters[0].Value = ID;

			Demo.Model.tb_JC_BOM model=new Demo.Model.tb_JC_BOM();
		...
		/// 得到一个对象实体
		/// </summary>
		public Demo.Model.tb_JC_BOM DataRowToModel(DataRow row)
		{
			Demo.Model.tb_JC_BOM model=new Demo.Model.tb_JC_BOM();
			if (row != null)
			{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,MaterialNO,MaterialType,CheckNO,CheckStatus,CheckDate,Status ");
            strSql.Append(" FROM tb_JC_BOM ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string sqlWhere, string order, int pageSize, int pageIndex, out long totalRecord)
		{
            ...
            DataSet ds = pageData.getDataPage("tb_JC_BOM", "*", sqlWhere, order, pageSize, pageIndex, out totalRecord);
            return ds;
		}
